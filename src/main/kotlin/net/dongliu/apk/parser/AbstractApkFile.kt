...
import net.dongliu.apk.parser.bean.ApkIcons.Raster
import net.dongliu.apk.parser.bean.ApkIcons.Adaptive
...
abstract class AbstractApkFile:Closeable {
     ...
     * @return icon files.
     */
    val icons: List<ApkIcons<*>> by lazy {
        xmlTranslator
        iconPaths.map { path ->
    ...
    private fun packagingIcon(
        ...
        if (density == Densities.Dynamic && filePath?.first() == '#' )
            return ApkIcons.Color(filePath)
        if (filePath.isNullOrEmpty()) return ApkIcons.empty
        val data = data ?: getFileData(filePath)
            ?: return ApkIcons.Empty(filePath)
        resourceTable
        ...
        return when {
            ...
            iconParser!=null -> {
                transBinaryXml(filePath)?.let { data -> ApkIcons.Vector(filePath,data) }
                    ?: ApkIcons.empty
            }
            else -> {
