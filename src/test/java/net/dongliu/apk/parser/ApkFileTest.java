...
public class ApkFileTest {
    ...
    public void testAppIsNotDebuggable() throws IOException {
        String path = Objects.requireNonNull(getClass().getClassLoader().getResource("apks/app-release.apk")).getPath();
        try (ApkFile apkFile = new ApkFile(path)) {
            ApkMeta apkMeta = apkFile.getApkMeta();
    ...
    public void testAppIsDebuggable() throws IOException {
        String path = Objects.requireNonNull(getClass().getClassLoader().getResource("apks/app-debug.apk")).getPath();
        try (ApkFile apkFile = new ApkFile(path)) {
            ApkMeta apkMeta = apkFile.getApkMeta();
